...
const Contact = () => {
  ...
  try {
    ...
    if (response.ok) {
      ...
      alert("Message sent successfully!"); // Or set a success state
      setFormData({ name: '', email: '', message: '' }); // Clear form
    } else {
      // 2. Handle errors returned by server
      throw new Error("Server responded with error");
    }
  } catch (error) {
    // 3. This block runs if fetch fails OR if you 'throw' inside the try block
    console.error("Error:", error);
    alert("Failed to send message."); // Make sure you aren't showing this on success!
  }
};
